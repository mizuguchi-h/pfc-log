...
- **AI相談**: Claude APIによるコーチ機能。目標・今日の記録・直近7日分が自動でコンテキストに入るので、状況説明なしで「今夜何食べるべき?」と聞ける
...
- キーは端末のlocalStorageにのみ保存されます(サーバーには送りません。API呼び出しはブラウザ→Anthropic直通)
- **個人利用前提の構成**です。公開URLを他人と共有する場合、キーが端末ごとに必要になるだけで漏洩はしませんが、自分のキーには[Consoleで利用上限(月額上限)を設定](https://console.anthropic.com/settings/limits)しておくのが安全です
- 本格的に運用するなら、将来的にはAPIキーをサーバー側(Vercel Functions等)に置くのが理想
