...
- ホーム画面に「今日の食事」として、食事タブで記録済みの内容をスロット(朝/昼/間食/夜)別に一覧表示する(閲覧のみ、編集/削除は食事タブで行う)
...
- 初回コミット(16ファイル)、kg対応、固定ルーティン反映、履歴の過去日編集、AI相談のGemini切り替え、食事記録のグラム数入力対応、食品リストの追加・削除UI、体重移動平均/記録ストリーク+見た目刷新、ジムメニューの自由なルーティン化(`menus`→`routines`)、重量アップ自動提案の削除、に続き、ホーム画面に「今日の食事」一覧を追加した
- ルーティン削除は割り当て済みの曜日を自動でオフ日にする(scheduleから参照を外す)仕様のため、「特定の曜日が急にオフ日になった」という報告があれば、まずルーティンを削除していないか(設定画面の「曜日ごとのジムメニュー」で該当曜日がオフ日になっていないか)を疑うこと
