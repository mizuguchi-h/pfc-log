...
- 初回コミット(16ファイル)、kg対応、固定ルーティン反映、履歴の過去日編集、AI相談のGemini切り替え、食事記録のグラム数入力対応、食品リストの追加・削除UI、体重移動平均/記録ストリーク+見た目刷新、ジムメニューの自由なルーティン化(`menus`→`routines`)、に続き、前回セットから自動で+2.5kgする「重量アップ自動提案」機能をオーナーの希望で削除した(表示のkgプリフィルは前回セットそのままに戻した)
